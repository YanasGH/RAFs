...
########## Hyperdeep Ensemble (Wenzel et al.) ##########
### Code mainly based on the repository for the paper "NOMU: Neural Optimization-based Model Uncertainty" by Heiss et al., which can be found at https://github.com/marketdesignresearch/NOMU ###
...
#Parameters
din = train.inputs.shape[1]
n_aug = 0; MCaug = False # No augmention points for all experiments
l2reg = 1e-8
FitHDE = True
layers_HDE = (din, 2 ** 6, 2 ** 8, 2 ** 7, 1)
epochs_HDE = 2000
batch_size_HDE = train_points
test_size_HDE = 0.2 
n_train_HDE = floor(train_points * (1 - test_size_HDE))  
c_HDE = 10
...
kappa_HDE = 5 #50
stratify_HDE = True 
fixed_row_init_HDE = True  
refit_HDE = False
softplus_min_var_HDE = (1e-6) 
learning_rate_HDE = None
random_locations = True 
static_parameters = OrderedDict(zip(["function_name", "dim", "n_train", "n_aug"], [dname, din, train_points, n_aug]))
...
def pretty_print_dict(D: Dict[str, any], printing: bool = True):
    text = []
...
def timediff_d_h_m_s(td: timedelta):
...
def gaussian_nll_hde(ytrue: tf.Tensor, ypred: tf.Tensor):
    ...
    constant = 0.5 * numpy.log(2 * numpy.pi)
...
class PredictionHistory_DE(Callback):
    ...
    def __init__(self, x_true: numpy.array, y_true: numpy.array):
    ...
    def on_epoch_begin(self, epoch: int, logs: Optional[Dict] = None):
        if self.params["verbose"] == 2:
            if epoch % 100 == 0:
                ...
                mu_pred = pred[:, 0]
                var_pred = pred[:, 1]
                print("\numpyrediction at epoch: {}: \n\n mu_pred:\n{}\n\n var_pred:\n{}\n\n ".format(epoch, mu_pred, var_pred))
                


def gaussian_nll_score(ytrue: numpy.array, mu_pred: numpy.array, sigma_pred: numpy.array,):
    ...
    b = numpy.log(sigma_pred)
    constant = 0.5 * numpy.log(2 * numpy.pi)
    nll = a + b + constant  # keep the constant to be able to compare the value
    return numpy.mean(nll)

def mse_score(ytrue: numpy.array, mu_pred: numpy.array,):
    return numpy.mean((ytrue - mu_pred) ** 2)
...
class HyperDeepEnsemble:
    ...
    def predict(
        ...
        -------
        predictions:
            A dictionary that stores the predictions for each model, e.g., for x = numpy.array([[x_1],[x_2]])
        ...
        for ensemble_key, ensemble in self.models.items():
            ...
            if p["loss"] == "nll":
                ...
                sum_mu_w = numpy.zeros((len(x), 1))
                sum_mu_squared_w = numpy.zeros((len(x), 1))
                sum_var_w = numpy.zeros((len(x), 1))
                ...
                std_pred = numpy.sqrt((sum_var_w + sum_mu_squared_w) - (sum_mu_w) ** 2)
                if verbose > 0:
                ...
                mu_pred = numpy.average(pred, axis=0, weights=weights)
                # calculate weighted std (ddof==0 version)
                std_pred = numpy.average(
            ...
            mu_pred = numpy.asarray(mu_pred, dtype=numpy.float32)
            std_pred = numpy.asarray(std_pred, dtype=numpy.float32)
            predictions[ensemble_key] = [mu_pred, std_pred]
        return predictions

    def calculate_mean_std(self, x: numpy.array) -> Dict[str, Tuple[numpy.array, numpy.array]]:
        ...
        -------
        predictions:
            A dictionary that stores the predictions for each model, e.g., for x = numpy.array([[x_1],[x_x]])
    ...
    def _random_search(
        self, ensemble_key, s: float = 0.05, activation: str = "tanh", verbose: int = 0
    ) -> NoReturn:
        # TODO: add description
    ...
    def _hyper_ens(
        ...
        score_current_ensemble = numpy.inf
        current_ensemble = []
        ...
        for i in range(1, self.parameters[ensemble_key]["kappa"] + 1):
            ...
            if isinstance(dropout_prob, tuple) and len(dropout_prob) == 2:
                u = numpy.random.uniform(
                    low=numpy.log10(dropout_prob[0]),
                    high=numpy.log10(dropout_prob[1]),
                    size=1,
                )[
                    0
                ]  # 'log' uniform
            ...
            if isinstance(l2reg, tuple) and len(l2reg) == 2:
                u = numpy.random.uniform(
                    low=numpy.log10(l2reg[0]), high=numpy.log10(l2reg[1]), size=1
                )[
                    0
                ]  # 'log' uniform
                u_factor = (
    ...
    def _calculate_ensemble_score(
        ...
        p = parameters
        if p["loss"] == "nll":
            sum_mu = numpy.zeros((len(x), 1))
            sum_mu_squared = numpy.zeros((len(x), 1))
            sum_var = numpy.zeros((len(x), 1))
            ...
            mu_pred = numpy.mean(pred, axis=0)
            std_pred = numpy.std(pred, axis=0)
        else:
...
for seed in [513]:
    ...
    # NB: Although setting multiple random seeds, this methods is not fully reprocible
    ...
    #Data
    b=numpy.array([0]*train_points)
    x_train = numpy.hstack([train.inputs,b.reshape(len(b),-1)])
    y_train = train.targets

    x_val = test.inputs
    y_val = test.targets
        
    x_aug = numpy.random.uniform(low=-1.1, high=1.1, size=(n_aug, din))
    y_aug = 10 * numpy.ones((n_aug, 1))
...
# display results
moments_HDE= hyper_deep_ensemble.calculate_mean_std(x=numpy.array(x_val))
method_means = moments_HDE['Hyper_Deep_Ensemble_1'][0].reshape(-1,)
method_stds = moments_HDE['Hyper_Deep_Ensemble_1'][1].reshape(-1,)
viz_one_d(dname, train, test, "Hyperdeep Ensemble", method_means, method_stds, predictions, False) # change to True if you want to save the plot

# report metrics
report_res(dname, test, y_val, "Hyperdeep Ensemble", method_means, method_stds, scaler_X, scaler_y)