...
########## Anchored Ensemble (Pearce et al.) ##########
###  Code mainly based on authors' repository, which can be found at https://github.com/TeaPearce/Bayesian_NN_Ensembles ####

# put the data into correct format
X_train = np.array(train.inputs)
y_train = np.array(train.targets)

X_val = np.array(test.inputs)
y_val = np.array(test.targets)
...
class NN():
    def __init__(self, x_dim, y_dim, hidden_size, init_stddev_1_w, init_stddev_1_b, 
        ...
        self.layer_1_w = tf.layers.Dense(hidden_size,
            activation=tf.nn.tanh, # try using tf.nn.relu, tf.erf, tf.nn.elu etc.
...
viz_one_d(dname, train, test, "Anchored Ensemble", method_means, method_stds, predictions, False) # change to True if you want to save the plot

# report metrics
report_res(dname, test, y_val, "Anchored Ensemble", method_means, method_stds, scaler_X, scaler_y)
